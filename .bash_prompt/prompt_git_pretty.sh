...
GIT_DYN_GRAPH_TPL_T=${GIT_DYN_GRAPH_TPL_T:-'%C(auto)%h%d %<|(__sw__,trunc)%s %C(bold blue)✏ %<(12,trunc)%aN %Creset%C(green)⏱  %<(12,trunc)%ar'}
...
prompt_git_dyn_graph_width() {
    ...
    if [[ $GIT_MSG_COLUMNS -ne $GIT_MSG_COLUMNS_OLD ]]; then
        ...
        git config -f "$configFile" pretty.graph-dyn-lg "${GIT_DYN_GRAPH_TPL_T//__sw__/${GIT_LG_COLUMNS}}"
    fi
}
